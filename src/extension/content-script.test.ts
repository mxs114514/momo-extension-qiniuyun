...
describe('content script overlay', () => {
  ...
  it('点击悬浮球后在固定尺寸面板中最多展示最新两句中文字幕', async () => {
    ...
    expect(panelStyle().left).toBe('50%')
    expect(panelStyle().bottom).toBe('8%')
    expect(panelStyle().transform).toBe('translateX(-50%)')
    expect(panelStyle().width).toBe('720px')
    expect(panelStyle().height).toBe('220px')
  })
